...
from fhir_server.elements.complex.codeableconcept import (
    CodeableConceptField, CodeableConcept as CodeableConceptDef)
...
class TestCodeableConcept(object):
    ...
    def test_post_data_with_null_coding_field(
            self, session, TestCodeableConceptModel):
        post = TestCodeableConceptModel(
            id=1,
            codeable_concept={
                'text': 'text'
    ...
    def test_fail_to_post_data_missing_fields(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ('Field text in column fhir_codeableconcept not '
                'nullable') in str(excinfo.value)
        assert ('Field coding in column fhir_codeableconcept not '
                'nullable') in str(excinfo.value)
