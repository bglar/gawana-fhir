...
from fhir_server.elements.complex.address import AddressField, Address as AddressDef
...
class TestAddress(object):
    ...
    def test_post_null_data_for_address_fields(self, session, TestAddressModel):
    ...
    def test_post_data_with_use_not_in_valueset(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "The address use must be defined in" in str(excinfo.value)
    ...
    def test_post_data_with_type_not_in_valueset(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "The address type must be defined in" in str(excinfo.value)
    ...
    def test_warning_if_country_value_not_valid_alpha3_ISO1366(
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            session.commit()
            assert len(w) == 1
            assert "This country code is not recommended." in str(w[-1].message)
    ...
    def test_nullable_change_to_false(self, session):
        ...
        district = [field for field in fields.columns if field.name == "district"]
        line = [field for field in fields.columns if field.name == "line"]
        postalCode = [field for field in fields.columns if field.name == "postalCode"]
    ...
    @patch("fhir_server.helpers.validations.requests.get")
    def test_post_data_field_city_present(self, mock_get, session, TestProfiledAddress):
        mock_get.return_value.json.return_value = {
            "count": 2,
            "data": [{"code": "home"}, {"code": "postal"}],
        }
    ...
    def test_fail_to_post_data_missing_city(self, session, TestProfiledAddress):
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "Field city in column fhir_address not nullable" in str(excinfo.value)
        assert "Field district in column fhir_address not nullable" in str(
            excinfo.value
        )
        assert "Field use in column fhir_address not nullable" in str(excinfo.value)
        assert "Field text in column fhir_address not nullable" in str(excinfo.value)
        assert "Field type in column fhir_address not nullable" in str(excinfo.value)

    def test_fail_to_post_data_missing_country(self, session, TestProfiledAddress):
        post = TestProfiledAddress(
            id=1,
            address={
                "use": "use",
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "Field country in column fhir_address not nullable" in str(excinfo.value)

    def test_fail_to_post_data_missing_district(self, session, TestProfiledAddress):
        post = TestProfiledAddress(
            id=1,
            address={
                "use": "home",
    ...
    def test_fail_to_post_data_missing_line(self, session, TestProfiledAddress):
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "Field line in column fhir_address not nullable" in str(excinfo.value)

    def test_fail_to_post_data_missing_postalCode(self, session, TestProfiledAddress):
        post = TestProfiledAddress(
            id=1,
            address={
                "use": "home",
    ...
    def test_fail_to_post_data_missing_state(self, session, TestProfiledAddress):
        post = TestProfiledAddress(
            id=1,
            address={
                "use": "home",
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "Field state in column fhir_address not nullable" in str(excinfo.value)
    ...
    def test_fail_to_post_data_missing_period(self, session, TestProfiledAddress):
        post = TestProfiledAddress(
            id=1,
            address={
                "use": "home",
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "Field period in column fhir_address not nullable" in str(excinfo.value)
