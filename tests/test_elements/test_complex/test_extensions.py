...
from fhir_server.elements.extension import ElementExtension, Extension as ExtensionDef


def test_opentype_coerced():
    assert OpenType.coerce_compared_value(OpenType(), "=", {"key": "value"})
...
class TestExtensions(object):
    ...
    def test_post_data(self, session, TestExtensionModel):
        ...
        post2 = TestExtensionModel(id=2, extension=None)
    ...
    def test_post_data_with_extension_as_none(self, session, TestExtensionModel):

        post2 = TestExtensionModel(
            id=2, extension={"url": "http://hl7.org/fhir/extensions", "value": None}
        )
    ...
    def test_reject_invalid_data(self, session, TestExtensionModel):
        ...
        assert ("The data provided for the extensions field is " "invalid") in str(
            excinfo.value
        )
    ...
    def test_post_data_fields_present(self, session, TestProfiledExtension):
    ...
    def test_fail_to_post_data_missing_fields(self, session, TestProfiledExtension):
        post = TestProfiledExtension(id=1, extension={})
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("Field url in column fhir_extension not " "nullable") in str(
            excinfo.value
        )
