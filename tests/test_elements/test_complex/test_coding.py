...
from fhir_server.elements.complex.coding import CodingField, Coding as CodingDef
...
class TestCoding(object):
    ...
    def test_reject_data_with_system_not_sanctioned_url(self, session, TestCodingModel):
        post = TestCodingModel(
            id=1,
            coding={
                "code": "dkls323-3223hj",
        ...
        with pytest.raises(StatementError) as excinfo:
            ...
            session.commit()
        assert (
            "Use a sanctioned uri from [http://hl7.org/fhir/"
    ...
    def test_nullable_change_to_false(self, session):
        ...
        version = [field for field in fields.columns if field.name == "version"]
    ...
    def test_post_data_fields_present(self, session, TestProfiledCoding):
    ...
    def test_fail_to_post_data_missing_fields(self, session, TestProfiledCoding):
        post = TestProfiledCoding(id=1, coding={})
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("Field code in column fhir_coding not " "nullable") in str(
            excinfo.value
        )
        assert ("Field display in column fhir_coding not " "nullable") in str(
            excinfo.value
        )
        assert ("Field system in column fhir_coding not " "nullable") in str(
            excinfo.value
        )
        assert ("Field userSelected in column fhir_coding not " "nullable") in str(
            excinfo.value
        )
        assert ("Field version in column fhir_coding not " "nullable") in str(
            excinfo.value
        )
