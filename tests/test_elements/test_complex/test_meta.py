...
class TestMeta(object):
    ...
    def test_post_data_with_null_meta_field(
            self, session, TestMetaModel):
        post = TestMetaModel(
            id=1,
            meta={}
        )
    ...
    def test_nullable_change_to_false(self, session):
        ...
        versionId = [
            field for field in fields.columns if field.name == 'versionId']
        lastUpdated = [
            field for field in fields.columns if field.name == 'lastUpdated']
        profile = [field for field in fields.columns if field.name == 'profile']
        security = [
            field for field in fields.columns if field.name == 'security']
    ...
    def test_fail_to_post_data_missing_fields(
        ...
        assert 'Field versionId in column fhir_meta not nullable' in str(
            excinfo.value)
        assert "Field lastUpdated in column fhir_meta not nullable" in str(
            excinfo.value)
        assert 'Field profile in column fhir_meta not nullable' in str(
            excinfo.value)
        assert 'Field security in column fhir_meta not nullable' in str(
            excinfo.value)
        assert 'Field tag in column fhir_meta not nullable' in str(
            excinfo.value)
