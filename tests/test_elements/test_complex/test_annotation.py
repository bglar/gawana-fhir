...
from fhir_server.elements.complex.annotation import (
    AnnotationField, Annotation as AnnotationDef)
...
class TestAnnotation(object):
    ...
    def test_nullable_change_to_false(self, session):
        ...
        authorReference = [
            field for field in fields.columns if (
                field.name == 'authorReference')]
    ...
    def test_fail_to_post_data_missing_city(
            self, session, TestProfiledAnnotation):
        post = TestProfiledAnnotation(
            id=1,
            annotation={
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ('Field authorString in column fhir_annotation not '
                'nullable') in str(excinfo.value)
        assert ('Field time in column fhir_annotation not '
                'nullable') in str(excinfo.value)
        assert ('Field text in column fhir_annotation not '
                'nullable') in str(excinfo.value)
    ...
    def test_fail_to_post_data_missing_authorReference(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ('Field authorReference in column fhir_annotation not '
                'nullable') in str(excinfo.value)
