...
from fhir_server.elements.complex.reference import (
    ReferenceField, Reference as ReferenceDef)
...
class TestReference(object):
    ...
    def test_reject_post_data_with_invalid_external_reference(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert 'We were unable to resolve a resource reference at' in str(
            excinfo.value)

    def test_post_data_with_null_reference_field(
            self, session, TestReferenceModel):
        post = TestReferenceModel(
            id=1,
            reference={}
    ...
    def test_nullable_change_to_false(self, session):
        ...
        reference = [
            field for field in fields.columns if field.name == 'reference']
    ...
    def test_fail_to_post_data_missing_fields(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ('Field reference in column fhir_reference not '
                'nullable') in str(excinfo.value)
        assert ('Field display in column fhir_reference not '
                'nullable') in str(excinfo.value)
