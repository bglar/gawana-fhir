...
class TestNarrative(object):
    ...
    @patch("fhir_server.helpers.validations.requests.get")
    def test_reject_data_with_invalid_tags(self, mock_get, session, TestNarrativeModel):
        mock_get.return_value.json.return_value = {
            "count": 2,
            "data": [{"code": "generated"}],
        }
    ...
    def test_reject_data_with_status_not_in_valuesets(
        ...
        post = TestNarrativeModel(
            id=1,
            narrative={"div": "<a>someattribute</a><b>strong</b>", "status": "status"},
        )
    ...
    def test_reject_content_containing_empty_string(
        ...
        post = TestNarrativeModel(id=1, narrative={"div": " ", "status": "generated"})
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "narrative content must not be an empty string" in str(excinfo.value)
