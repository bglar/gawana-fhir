...
from fhir_server.elements.complex.period import PeriodField, Period as PeriodDef
...
class TestPeriod(object):
    ...
    def test_post_data(self, session, TestPeriodModel):
        post = TestPeriodModel(
            id=1, period={"start": "2011-05-24", "end": "2011-06-24"}
        )

        post2 = TestPeriodModel(id=2, period=None)
    ...
    def test_posting_str_data_on_composite_fields(self, session, TestPeriodModel):
        post = TestPeriodModel(id=1, period="test")

        post2 = TestPeriodModel(id=2)
    ...
    def test_nullable_change_to_false(self, session):
        ...
        start = [field for field in fields.columns if field.name == "start"]

        assert not start[0].nullable
        assert not end[0].nullable

    def test_post_data_fields_present(self, session, TestProfiledPeriod):
    ...
    def test_fail_to_post_data_missing_fields(self, session, TestProfiledPeriod):
        post = TestProfiledPeriod(id=1, period={})
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("Field start in column fhir_period not " "nullable") in str(
            excinfo.value
        )
        assert ("Field end in column fhir_period not " "nullable") in str(excinfo.value)
