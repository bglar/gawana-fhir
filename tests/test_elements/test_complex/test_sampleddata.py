...
class TestSampledData(object):
    ...
    def test_post_data_with_null_sampleddata_field(self, session, TestSampledDataModel):
        post = TestSampledDataModel(
            id=1,
            sampleddata={
                "data": "2.3U",
    ...
    def test_fail_to_post_data_if_there_is_more_than_one_whitespace(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("data in sampleddata must not have more than 1 " "whitespace") in str(
            excinfo.value
        )
    ...
    def test_nullable_change_to_false(self, session):
        ...
        lowerLimit = [field for field in fields.columns if field.name == "lowerLimit"]
        upperLimit = [field for field in fields.columns if field.name == "upperLimit"]

        assert not factor[0].nullable
        assert not lowerLimit[0].nullable
        assert not upperLimit[0].nullable

    def test_post_data_fields_present(self, session, TestProfiledSampledData):
        post = TestProfiledSampledData(
            id=1,
            sampleddata={
                "data": "2.3L",
    ...
    def test_fail_to_post_data_missing_fields(self, session, TestProfiledSampledData):
        post = TestProfiledSampledData(
            id=1,
            sampleddata={
                "data": "2.5 ",
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("Field factor in column fhir_sampleddata not " "nullable") in str(
            excinfo.value
        )
        assert ("Field lowerLimit in column fhir_sampleddata not " "nullable") in str(
            excinfo.value
        )
        assert ("Field upperLimit in column fhir_sampleddata not " "nullable") in str(
            excinfo.value
        )
