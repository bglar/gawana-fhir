...
class TestHumanName(object):
    ...
    def test_post_data_with_whitespace_on_family_fails(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "family in name must not have a whitespace" in str(excinfo.value)
    ...
    def test_post_data_with_whitespace_on_given_fails(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "given in name must not have a whitespace" in str(excinfo.value)
    ...
    def test_post_data_with_whitespace_on_prefix_fails(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "prefix in name must not have a whitespace" in str(excinfo.value)
    ...
    def test_post_data_with_whitespace_on_suffix_fails(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "suffix in name must not have a whitespace" in str(excinfo.value)
    ...
    def test_post_data_with_use_not_defined_in_valuesets(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "The humanname use must be defined in" in str(excinfo.value)

    def test_post_data_with_null_humanname_field(self, session, TestHumanNameModel):
        post = TestHumanNameModel(id=1, humanname={})
    ...
    def test_fail_to_post_data_missing_fields(self, session, TestProfiledHumanName):
        post = TestProfiledHumanName(id=1, humanname={})
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("Field family in column fhir_humanname not " "nullable") in str(
            excinfo.value
        )
        assert ("Field given in column fhir_humanname not " "nullable") in str(
            excinfo.value
        )
        assert ("Field prefix in column fhir_humanname not " "nullable") in str(
            excinfo.value
        )
        assert ("Field suffix in column fhir_humanname not " "nullable") in str(
            excinfo.value
        )
        assert ("Field text in column fhir_humanname not " "nullable") in str(
            excinfo.value
        )
        assert ("Field use in column fhir_humanname not " "nullable") in str(
            excinfo.value
        )
        assert ("Field period in column fhir_humanname not " "nullable") in str(
            excinfo.value
        )
