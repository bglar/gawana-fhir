...
class TestIdentifier(object):
    ...
    def test_post_data_with_type_code_not_in_valuesets(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "The identifier type must be defined in" in str(excinfo.value)
    ...
    def test_post_data_with_null_identifier_field(self, session, TestIdentifierModel):
        post = TestIdentifierModel(id=1, identifier={})
    ...
    def test_nullable_change_to_false(self, session):
        ...
        assigner = [field for field in fields.columns if field.name == "assigner"]
    ...
    @patch("fhir_server.helpers.validations.requests.get")
    def test_post_data_fields_present(self, mock_get, session, TestProfiledIdentifier):
        mock_get.return_value.json.return_value = {
            "count": 2,
            "data": [{"code": "secondary"}, {"code": "UDI"}],
        }
    ...
    def test_fail_to_post_data_missing_fields(self, session, TestProfiledIdentifier):
        post = TestProfiledIdentifier(id=1, identifier={})
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("Field system in column fhir_identifier not " "nullable") in str(
            excinfo.value
        )
        assert ("Field use in column fhir_identifier not " "nullable") in str(
            excinfo.value
        )
        assert ("Field value in column fhir_identifier not " "nullable") in str(
            excinfo.value
        )
        assert ("Field assigner in column fhir_identifier not " "nullable") in str(
            excinfo.value
        )
        assert ("Field period in column fhir_identifier not " "nullable") in str(
            excinfo.value
        )
        assert ("Field type in column fhir_identifier not " "nullable") in str(
            excinfo.value
        )
