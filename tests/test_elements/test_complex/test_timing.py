...
from fhir_server.elements.complex.timing import TimingField, Timing as TimingDef
...
class TestTiming(object):
    ...
    def test_post_data(self, mock_get, session, TestTimingModel):
        mock_get.return_value.json.return_value = {
            "count": 1,
            "data": [{"code": "QOD"}, {"code": "min"}, {"code": "code"}],
        }
        post = TestTimingModel(
            id=1,
            timing={
                "event": ["2015-10-10T11:11:11+02:00", "2015-10-10T11:12:11+02:00"],
                "code": {
                    "text": "text",
                    "coding": [
                        {
                ...
                "repeat": {
                    ...
                    "periodMax": 7.59,
                    "periodUnits": "min",
                    "boundsPeriod": {"start": "2015-10-10", "end": "2015-11-11"},
                    "boundsQuantity": {
                        "code": "code",
                        "system": "system",
                        "unit": "kg",
    ...
    def test_post_fails_if_duration_present_but_no_durationUnits(
        ...
        post = TestTimingModel(
            id=1,
            timing={
                "event": ["2015-10-10T11:11:11+02:00", "2015-10-10T12:11:11+02:00"],
                "code": {
                    "text": "text",
                    "coding": [
                        {
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("durationUnits must be present for the duration " "provided") in str(
            excinfo.value
        )
    ...
    def test_post_fails_if_period_present_but_no_periodUnits(
        ...
        post = TestTimingModel(
            id=1,
            timing={
                "event": ["2015-10-10T11:11:11+02:00", "2015-10-10T10:11:11+02:00"],
                "code": {
                    "text": "text",
                    "coding": [
                        {
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("periodUnits must be present for the period " "provided") in str(
            excinfo.value
        )
    ...
    def test_post_fails_if_both_frequency_and_when_exists(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("Either frequency or when can exist, not both") in str(excinfo.value)
    ...
    def test_period_must_be_provided_if_periodMax_is_provided(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "If there is a periodMax, there must be a period" in str(excinfo.value)
    ...
    def test_post_fails_if_duration_unit_provided_is_not_in_valueset(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("The timing durationUnits must be defined in") in str(excinfo.value)
    ...
    def test_post_fails_if_period_unit_provided_is_not_in_valueset(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "The timing periodUnits must be defined in" in str(excinfo.value)
    ...
    def test_post_fails_if_when_value_provided_is_not_in_valueset(
        ...
        mock_get.return_value.json.return_value = {
            "count": 1,
            "data": [{"code": "QOD"}, {"code": "min"}, {"code": "s"}, {"code": "code"}],
        }
        post = TestTimingModel(
            id=1,
            timing={
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "The timing when must be defined in" in str(excinfo.value)
    ...
    def test_post_fails_if_code_value_provided_is_not_in_valueset(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "The timing code must be defined in" in str(excinfo.value)

    def test_post_data_with_null_timing_field(self, session, TestTimingModel):
        post = TestTimingModel(id=1, timing={})
    ...
    @patch("fhir_server.helpers.validations.requests.get")
    def test_post_data_fields_present(self, mock_get, session, TestProfiledTiming):
        mock_get.return_value.json.return_value = {
            "count": 1,
            "data": [{"code": "QOD"}, {"code": "min"}],
        }
    ...
    def test_fail_to_post_data_missing_fields(self, session, TestProfiledTiming):
        post = TestProfiledTiming(id=1, timing={})
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ("Field code in column fhir_timing not " "nullable") in str(
            excinfo.value
        )
        assert ("Field event in column fhir_timing not " "nullable") in str(
            excinfo.value
        )
        assert ("Field repeat in column fhir_timing not " "nullable") in str(
            excinfo.value
        )
