...
from fhir_server.elements.complex.range import (
    RangeField, Range as RangeDef)
...
class TestRange(object):
    ...
    def test_post_fails_if_high_and_low_units_dont_match(
    ...
    def test_post_fails_if_high_and_low_code_dont_match(
            self, session, TestRangeModel):
        post = TestRangeModel(
            id=1,
            range={
    ...
    def test_post_data_with_null_range_field(
            self, session, TestRangeModel):
        post = TestRangeModel(
            id=1,
            range={}
    ...
    def test_post_data_fields_present(
            self, session, TestProfiledRange):
    ...
    def test_fail_to_post_data_missing_fields(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ('Field high in column fhir_range not '
                'nullable') in str(excinfo.value)
        assert ('Field low in column fhir_range not '
                'nullable') in str(excinfo.value)
