...
class TestSignature(object):
    ...
    def test_post_data(self, mock_get, session, TestSignatureModel):
        mock_get.return_value.json.return_value = {
            "count": 1,
            "data": [{"code": "application/jwt"}, {"code": "1.2.840.10065.1.12.1.1"}],
        }
        post = TestSignatureModel(
            ...
            signature={
                ...
                "whoUri": "whoUri",
                "whoReference": {"reference": "reference url", "display": "Patient X"},
                "type": [
                    {
    ...
    def test_reject_data_with_contentType_not_valid(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "The signature content type should be one of" in str(excinfo.value)

    @patch("fhir_server.helpers.validations.requests.get")
    def test_reject_invalid_jwt_blob(self, mock_get, session, TestSignatureModel):
        mock_get.return_value.json.return_value = {
            "count": 1,
            "data": [{"code": "application/jwt"}, {"code": "1.2.840.10065.1.12.1.1"}],
        }
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert "The blob provided is not a valid jwt encoded" in str(excinfo.value)

    def test_excluding_required_fields_in_post(self, session, TestSignatureModel):
        post = TestSignatureModel(
            id=1,
            signature={
                "contentType": "application/jwt",
        ...
        assert ("Field blob in column fhir_signature not " "nullable") in str(
            excinfo.value
        )
