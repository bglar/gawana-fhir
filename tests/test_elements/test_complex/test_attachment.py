...
from fhir_server.elements.complex.attachment import (
    AttachmentField, Attachment as AttachmentDef)
...
class TestAttachment(object):
    ...
    def test_rejects_data_with_invalid_contentType(
    ...
    def test_post_fails_if_language_code_provided_is_not_valid(
        ...
        assert 'the language code must be valid' in str(
            excinfo.value)
    ...
    def test_post_fails_if_hash_is_not_b64_encode_sha1_of_data(
        ...
        assert 'the hash must be a base64 sha-1 hash of the data' in str(
            excinfo.value)

    def test_post_data_with_null_attachment_field(
            self, session, TestAttachmentModel):
        post = TestAttachmentModel(
            id=1,
            attachment={}
    ...
    def test_nullable_change_to_false(self, session):
        ...
        contentType = [
            field for field in fields.columns if field.name == 'contentType']
        creation = [
            field for field in fields.columns if field.name == 'creation']
        data = [field for field in fields.columns if field.name == 'data']
        language = [
            field for field in fields.columns if field.name == 'language']
    ...
    def test_post_data_required_fields_present(
    ...
    def test_fail_to_post_data_missing_required_fields(
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert ('Field contentType in column fhir_attachment not '
                'nullable') in str(excinfo.value)
        assert 'Field creation in column fhir_attachment not nullable' in str(
            excinfo.value)
        assert 'Field data in column fhir_attachment not nullable' in str(
            excinfo.value)
        assert 'Field language in column fhir_attachment not nullable' in str(
            excinfo.value)
        assert 'Field hash in column fhir_attachment not nullable' in str(
            excinfo.value)
        assert 'Field size in column fhir_attachment not nullable' in str(
            excinfo.value)
        assert 'Field title in column fhir_attachment not nullable' in str(
            excinfo.value)
        assert 'Field url in column fhir_attachment not nullable' in str(
            excinfo.value)
