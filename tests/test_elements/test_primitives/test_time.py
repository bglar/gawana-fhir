...
class TestTimeField(object):
    ...
    def test_time_with_timezone(self):
        with pytest.raises(ValueError) as excinfo:
            primitives.TimeField().process_bind_param("22:00:00+00:00", "postgres")
    ...
    def test_TimeField_with_date(self, session, TestDataTypesModel):
        now = datetime.now()
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), time_field=str(now.date()))

        with pytest.raises(StatementError) as excinfo:
            session.add(post_data)
            session.commit()

        assert "Time field expects str or time object but %s was given" % type(
            str(now.date()) in str(excinfo.value)
        )

    def test_time_accept_none_values(self, session, TestDataTypesModel):
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), time_field=None)
    ...
    def test_time_accept_time_object(self, session, TestDataTypesModel):
        now = datetime.now()
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), time_field=now.time())
