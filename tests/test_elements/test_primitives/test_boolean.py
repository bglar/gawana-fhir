...
class TestBooleanField(object):
    ...
    def test_true_value(self, session, TestDataTypesModel):
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), bool_field=True)
    ...
    def test_false_value(self, session, TestDataTypesModel):
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), bool_field=False)
        session.add(post_data)
        session.commit()
    ...
    def test_must_not_accept_1(self, session, TestDataTypesModel):
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), bool_field=1)
        session.add(post_data)
        with pytest.raises(StatementError) as excinfo:
            session.commit()

        assert "1 is an invalid value for the fhir boolean field" in str(excinfo.value)

    def test_must_not_accept_0(self, session, TestDataTypesModel):
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), bool_field=0)
        session.add(post_data)
        with pytest.raises(StatementError) as excinfo:
            session.commit()

        assert "0 is an invalid value for the fhir boolean field" in str(excinfo.value)
