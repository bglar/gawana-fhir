...
class TestPositiveIntField(object):
    def test_positive_num(self):
        result = primitives.PositiveIntField().process_bind_param(10, "postgres")
        assert result == 10
    ...
    def test_zero(self):
        ...
        assert "Value 0 must be a positive integer greater than 0" in str(excinfo.value)

    def test_none(self):
        result = primitives.PositiveIntField().process_bind_param(None, "postgres")
    ...
    def test_correct_data(self, session, TestDataTypesModel):
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), pos_int_field=12)
    ...
    def test_value_less_than_0(self, session, TestDataTypesModel):
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), pos_int_field=-10)
    ...
    def test_value_equal_0(self, session, TestDataTypesModel):
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), pos_int_field=0)
