...
class TestDateField(object):
    ...
    def test_only_month_and_year(self):
        result = primitives.DateField().process_bind_param(
            '2011-05', 'postgres')
    ...
    def test_time(self):
        with pytest.raises(TypeError) as excinfo:
            primitives.DateField().process_bind_param(
                '2011-05-24T23:00:00', 'postgres')

        assert ('The Date 2011-05-24T23:00:00 is invalid') in str(
            excinfo.value)
    ...
    def test_DateField_invalid(self, session, TestDataTypesModel):
        ...
        assert ('The Date %s is invalid' % post_data.date_field) in str(
            excinfo.value)
