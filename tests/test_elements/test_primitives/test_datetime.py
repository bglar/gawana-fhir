...
class TestDateTimeField(object):
    def test_only_year(self):
        result = primitives.DateTimeField().process_bind_param("2011", "postgres")
        assert result == "2011"

    def test_only_month_and_year(self):
        result = primitives.DateTimeField().process_bind_param("2011-05", "postgres")
        assert result == "2011-05"

    def test_only_day_month_and_year(self):
        result = primitives.DateTimeField().process_bind_param("2011-05-24", "postgres")
    ...
    def test_time_to_seconds_no_timezone(self):
        ...
        assert "The DateTime 2011-05-24T22:00:02 is invalid" in str(excinfo.value)
    ...
    def test_time_no_seconds(self):
        ...
        assert "The DateTime 2011-05-24T22:00 is invalid" in str(excinfo.value)
    ...
    def test_not_allow_2400_hrs(self):
        ...
        assert "The DateTime 2011-05-24T24:00:00 is invalid" in str(excinfo.value)
    ...
    def test_invalid_time_is_rejected(self, session, TestDataTypesModel):
        ...
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), date_time_field=siku_mbaya)

        with pytest.raises(StatementError) as excinfo:
            session.add(post_data)
            session.commit()

        assert ("The DateTime %s is invalid" % siku_mbaya) in str(excinfo.value)

    def test_DateTimeField_no_timezone(self, session, TestDataTypesModel):
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), date_time_field="Sema")
    ...
    def test_seconds_prefilled(self, session, TestDataTypesModel):
        id = str(uuid.uuid4())
        post_data = TestDataTypesModel(id=id, date_time_field="2011-05-24T22:25+03:00")
        session.add(post_data)
        session.commit()
