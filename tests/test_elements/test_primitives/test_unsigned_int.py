...
class TestUnsignedIntField(object):
    def test_positive_num(self):
        result = primitives.UnsignedIntField().process_bind_param(10, "postgres")
        assert result == 10
    ...
    def test_negative_num(self):
        ...
        assert "Value -10 must be an int greater than or equal 0" in str(excinfo.value)
    ...
    def test_none_values(self):
        result = primitives.UnsignedIntField().process_bind_param(None, "postgres")
    ...
    def test_value_less_than_0(self, session, TestDataTypesModel):
        post_data = TestDataTypesModel(id=str(uuid.uuid4()), uns_int_field=-10)
