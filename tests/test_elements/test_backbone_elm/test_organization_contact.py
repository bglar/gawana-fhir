...
class TestOrganizationContact(object):
    ...
    def test_post_data(self, mock_get, session, TestOrganizationContactModel):
        mock_get.return_value.json.return_value = {
            "count": 1,
            "data": [{"code": "BILL"}, {"code": "official"}, {"code": "home"}],
        }
    ...
    def test_fail_to_post_data_missing_fields(
        ...
        post = TestProfiledOrganizationContact(id=1, organizationcontact={})
        ...
        with pytest.raises(StatementError) as excinfo:
            session.commit()
        assert (
            "Field purpose in column fhir_organizationcontact not " "nullable"
        ) in str(excinfo.value)
        assert ("Field name in column fhir_organizationcontact not " "nullable") in str(
            excinfo.value
        )
        assert (
            "Field telecom in column fhir_organizationcontact not " "nullable"
        ) in str(excinfo.value)
        assert (
            "Field address in column fhir_organizationcontact not " "nullable"
        ) in str(excinfo.value)
