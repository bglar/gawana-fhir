...
class TestOrganizationApis(object):
    ...
    def test_post_ignores_meta_version_and_last_modified(self, client):
        ...
        records = {"name": "Test Organization", "meta": self.meta}
        response = client.post(
            self.ORG_URL, content_type="application/json", data=json.dumps(records)
        )
        data = json.loads(response.get_data().decode("utf8"))
    ...
    def test_post_with_single_field(self, client):
        records = {"name": "Test Organization"}
        response = client.post(
            self.ORG_URL, content_type="application/json", data=json.dumps(records)
        )
        data = json.loads(response.get_data().decode("utf8"))
    ...
    def test_get_summary_returns_slim_resource_instance(self, client, new_org):
        ...
        get_text = client.get(self.ORG_URL + "/1", query_string=q_param_data)
        data = json.loads(get_text.get_data().decode("utf8"))
        assert data.get("language") == "EN"
        assert data.get("meta")
    ...
    def test_update_fails_with_404_if_resource_not_found(self, client, new_org):
        put = client.put(
            self.ORG_URL + "/22",
            content_type="application/json",
            data=json.dumps({"name": "Change Me"}),
    ...
    def test_conditional_update_rejected_if_no_results_found(self, client, new_org):
    ...
    def test_post_fails_with_invalid_data(self, client):
        records = {"namessss": "Test Organization"}
        request = client.post(
            self.ORG_URL, content_type="application/json", data=json.dumps(records)
        )
    ...
    def test_post_fails_if_id_is_provided(self, client):
        records = {"id": "122", "name": "Test Organization"}
        request = client.post(
            self.ORG_URL, content_type="application/json", data=json.dumps(records)
        )
