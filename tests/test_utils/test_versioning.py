...
class TestVersioning(AssertsCompiledSQL):
    ...
    def test_w_mapper_versioning(self, session1, SomeClass):
        SomeClass.metadata.create_all(session1.connection())
        SomeClass.__mapper__.version_id_col = \
            SomeClass.__table__.c.resource_version
    ...
    def test_joined_inheritance_multilevel(self, BaseClass, SubClass,
        ...
        eq_(
            ...
            [SubSubHistory(name='ss1', subdata1='sd1',
                           subdata2='sd2', type='subsub', resource_version=1)]
        )
        eq_(ssc, SubSubClass(
            name='ss1', subdata1='sd11',
            subdata2='sd22', resource_version=2))
    ...
    def test_joined_inheritance_changed(self, BaseClass, JoinSubClass,
        ...
        actual_changed_base = sess.scalar(select(
            [BaseClass.__history_mapper__.local_table.c.resource_changed]))
        actual_changed_sub = sess.scalar(select(
    ...
    def test_single_inheritance(self, InheritanceSubClass,
                                BaseClass, session1):
        BaseClass.metadata.create_all(session1.connection())
        ...
        InheritanceSubClassHistory = (
            InheritanceSubClass.__history_mapper__.class_)
        ...
        eq_(
            sess.query(BaseClassHistory).order_by(
                BaseClassHistory.id, BaseClassHistory.resource_version).all(),
            [
                BaseClassHistory(id=1, name='b1', type='base',
                                 resource_version=1),
                BaseClassHistory(
