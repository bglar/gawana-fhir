...
def _response_to_xml(data):
    ...
    if isinstance(resource_dict, list):
        ...
        for data in resource_dict:
            resource_type = str(data.pop("resourceType")["@value"]).title()

            data["@xmlns"] = "http://hl7.org/fhir"
            resource = {"resource": {resource_type: data}}
            resource_list.append(resource)
        resource_xml = xmltodict.unparse(
            {"Bundle": {"@xmlns": "http://hl7.org/fhir", "entry": resource_list}}
        )
    else:
        # remove the resourceType from xml and make it the xml root
        resource_type = str(resource_dict.pop("resourceType")["@value"]).title()
...
def _make_response(response, code=200, mime_type="text/html", **kwargs):
    ...
    if isinstance(response_data, list):
        ...
        response_data = {"resourceType": "Bundle", "total": count, "entry": data}

    if str(mime_type[0]) in VALID_XML_MIMETYPES:
        mime_type = "application/xml"
        response_data = _response_to_xml(response_data)
        api_response = Response(
            response_data, code, mimetype=mime_type, content_type=mime_type
        )
...
def make_error_resp(error, msg=None, code=400, mime_type="text/html"):
    ...
    try:
        ...
        response = {"OperationOutcome": log, "DEBUG_LOG": msg}
...
def make_exception_resp(exception, code=422, mime_type="text/html"):
    ...
    if current_app.config["DEBUG"]:
        return make_error_resp(exception, msg=msg, code=code, mime_type=mime_type)
    else:
...
def log_operation_outcome(
    ...
    try:
        ...
        return make_exception_resp(e.args, code=status_code, mime_type=mime_type)

    return make_data_resp(
        outcome_response,
        OperationOutcome,
        code=status_code,
        mime_type=mime_type,
