...
class BaseApi(object):
    ...
    def get_resource(self):
        ...
        ), "{0} should include a `resource` attribute".format(self.__class__.__name__)

        assert isinstance(
            self.resource, _BoundDeclarativeMeta
...
def validate_resource_type(request_data, location, mime_type):
    ...
    resource_names = [resource.__tablename__.lower() for resource in all_resources]
    response = None

    if not resource_type:
        err = (
            "Failed to parse request body as resource. Error was: "
...
class CreateMixin(BaseApi, views.MethodView):
    ...
    def post(self, key=None, operation=None):
        ...
        if not request.data:
            err = "No data provided." "You cannot create an empty resource instance."
            return Response.log_operation_outcome(
                [location], code="invalid", diagnostics=err, mime_type=mime_type
            )

        if request.data.__contains__("id"):
            id = request.data["id"]
            err = (
                "Can not create resource with ID {0}, ID must not be "
...
class RetrieveMixin(BaseApi, views.MethodView):
    ...
    def get(self, key, vid=None):
        ...
        if not query:
            err = (
                "{0}/{1}/_history/{2} is not known".format(
                    resource.__tablename__.title(), key, vid
                )
                if vid
                else "{0}/{1} is not known".format(resource.__tablename__.title(), key)
            )
        ...
        return Response.make_data_resp(
            query, resource, code=status_code, mime_type=mime_type, location=base_url
        )
    ...
    def delete(self, key=None):
        ...
        if not key:
            if not len(request_args) > 0:
                err = (
                    "Logical ID or QueryParam(for conditional deletes) " "not provided "
                )
                return Response.log_operation_outcome(
                    location=location,
                    code="invalid",
            ...
            if status_code != 200:
                return Response.log_operation_outcome(
                    [location],
                    diagnostics=HTTP_STATUS_CODES.get(status_code),
                    status_code=status_code,
                    mime_type=mime_type,
                )
        ...
        if not query:
            err = "{0}/{1}/ is not known".format(resource.__tablename__, key)
            return Response.log_operation_outcome(
                [location + "id"],
                code="invalid",
                diagnostics=err,
...
class UpdateMixin(BaseApi, views.MethodView):
    def put(self, key=None, vid=None):
        ...
        if not request.data or (len(request.data) == 0):
            ...
            return Response.log_operation_outcome(
                [location], code="invalid", mime_type=mime_type, diagnostics=err
            )
        ...
        if not key:
            if not len(request_args) > 0:
                err = (
                    "Logical ID or QueryParam(for conditional updates) " "not provided "
                )
                return Response.log_operation_outcome(
                    location=location,
                    code="invalid",
        ...
        if query and vid:
            ...
            if not resource_version == vid:
                err = (
                    "Trying to update {}/{}/_history/{} but this is not the "
                    "current version".format(resource.__tablename__, key, vid)
                )
                return Response.log_operation_outcome(
                    [location], diagnostics=err, status_code=400, mime_type=mime_type
                )
        ...
        if if_match:
            ...
            err = (
                "Pre-condition failed. An If-Match header should "
        ...
        try:
            ...
            if status_code != 200:
                Response.log_operation_outcome(
                    [location],
                    diagnostics=HTTP_STATUS_CODES[status_code],
                    status_code=status_code,
                    mime_type=mime_type,
                )

        return Response.make_data_resp(
            data, resource, code=200, mime_type=mime_type, location=base_url
        )
