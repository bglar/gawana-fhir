class MetaOperations(object):
    ...
    def meta_list(cls, **kwargs):
        ...
        def construct_meta(resource, results):
            if results:
                ...
                resource_meta = [resource(meta=dict_resource.get(
                    'meta')) for dict_resource in dict_resources]
    ...
    def meta_add(cls, key, data, **kwargs):
        ...
        try:
            meta = data.get('meta')
            if meta and (meta.get('security') or
                         meta.get('tag') or
                         meta.get('profile')):
                ...
                if query:
                    ...
                    raise ValueError('Resource {}/{} is not known'.format(
                        cls.__tablename__, key))
            else:
