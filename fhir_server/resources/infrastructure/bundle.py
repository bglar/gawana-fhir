...
class BundleEntryRequest(BackboneElement):
    ...
    def element_properties(self):
        ...
        elm.extend([
            Field('method', {'mini': 1, 'maxi': 1},
                  primitives.CodeField, None),
            # GET | POST | PUT | DELETE.

            Field('url', {'mini': 1, 'maxi': 1},
                  primitives.URIField, None),
            # URL for HTTP equivalent of this entry.

            Field('ifNoneMatch', {'mini': 0, 'maxi': 1},
                  primitives.StringField, None),
            # For managing cache currency.

            Field('ifModifiedSince', {'mini': 0, 'maxi': 1},
                  primitives.InstantField, None),
            # For managing update contention.

            Field('ifMatch', {'mini': 0, 'maxi': 1},
                  primitives.StringField, None),
            # For managing update contention.

            Field('ifNoneExist', {'mini': 0, 'maxi': 1},
                  primitives.StringField, None),
...
class BundleEntryResponse(BackboneElement):
    ...
    def element_properties(self):
        elm = super().element_properties()
        elm.extend([
            Field('etag', {'mini': 0, 'maxi': 1},
                  primitives.StringField, None),
            # The etag for the resource (if relevant).

            Field('lastModified', {'mini': 0, 'maxi': 1},
                  primitives.InstantField, None),
            ...
            Field('status', {'mini': 1, 'maxi': 1},
                  primitives.CodeField, None),
            # Status return code for entry.

            Field('outcome', {'mini': 1, 'maxi': 1},
                  complex.ReferenceField(), ['OperationOutcome'])
        ])
        return elm
...
class BundleEntry(BackboneElement):
    ...
    def element_properties(self):
        elm = super().element_properties()
        elm.extend([
            Field('fullUrl', {'mini': 0, 'maxi': 1},
                  primitives.URIField, None),
            # Absolute URL for resource (server address, or UUID/OID).

            Field('link', {'mini': 0, 'maxi': -1},
                  BundleLinkField(), None),
            # Links related to this entry.

            Field('request', {'mini': 0, 'maxi': 1},
                  BundleEntryRequestField(), None),
            # Transaction Related Information.

            Field('response', {'mini': 0, 'maxi': 1},
                  BundleEntryResponseField(), None),
            # Transaction Related Information.

            Field('search', {'mini': 0, 'maxi': 1},
                  BundleEntrySearchField(), None),
            # Search related information.

            Field('resource', {'mini': 0, 'maxi': 1},
                  complex.ReferenceField(), 'all'),
