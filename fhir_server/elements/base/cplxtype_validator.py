...
from fhir_server.helpers.validations import validate_valuesets
...
from fhir_server.configs import (
    ADDRESS_USE_URL,
    ADDRESS_TYPE_URL,
    QUANTITY_COMPARATOR_URL,
    HUMANNAME_USE_URL,
    UNITS_OF_TIME_URL,
    EVENT_TIMING_URL,
    TIMING_ABBREVIATION_URL,
    CONTACT_POINT_SYSTEM_URL,
    CONTACT_POINT_USE_URL,
    SIGNATURE_TYPE_URL,
    IDENTIFIER_TYPE_URL,
    IDENTIFIER_USE_URL,
    UCUM_SYSTEM_URI,
...
class CompositeValidator(object):
    ...
    def validate_fhir_attachment(self):
        ...
        if self.values.data:
            if not self.values.contentType:
                errors.append("content_type must be populated if data is provided")
        ...
        if self.values.hash and self.values.data:
            ...
            if not self.values.hash == valid_hash:
                errors.append("the hash must be a base64 sha-1 hash of the data")
    ...
    def validate_fhir_simplequantity(self):
        if self.values.code:
            if not self.values.system:
                raise ValueError("system must be specified if code is provided")
    ...
    def validate_fhir_quantity(self):
        ...
        if self.values.comparator:
            ...
            validate_valuesets(self.values.comparator, url, "quantity comparator")
    ...
    def validate_fhir_money(self):
        ...
        if self.values.value:
            if not self.values.code:
                raise ValueError("code must be specified if value is provided")
        return self.values
    ...
    def validate_fhir_timing(self):
        ...
        if self.values.repeat:
            if self.values.repeat.duration and not (self.values.repeat.durationUnits):
                errors.append("durationUnits must be present for the duration provided")

            if self.values.repeat.periodMax:
                if not self.values.repeat.period:
                    errors.append("If there is a periodMax, there must be a period")
            ...
            if self.values.repeat.period:
                if not self.values.repeat.periodUnits:
                    errors.append("periodUnits must be present for the period provided")
            ...
            if self.values.repeat.durationMax:
                if not self.values.repeat.duration:
                    errors.append("If there is a durationMax, there must be a duration")
                if self.values.repeat.durationMax < 0:
                    errors.append("durationMax SHALL be a non-negative value")

            if self.values.repeat.duration and (self.values.repeat.duration < 0):
                errors.append("duration SHALL be a non-negative value")

            if self.values.repeat.durationUnits:
                url = UNITS_OF_TIME_URL + "?code=" + (self.values.repeat.durationUnits)
                validate_valuesets(
                    self.values.repeat.durationUnits, url, "timing durationUnits"
                )

            if self.values.repeat.periodUnits:
                url = UNITS_OF_TIME_URL + "?code=" + (self.values.repeat.periodUnits)
                validate_valuesets(
                    self.values.repeat.periodUnits, url, "timing periodUnits"
                )
        ...
        if self.values.code and self.values.code.coding:
            ...
            for i, val in enumerate(self.values.code.coding):
                ...
                validate_valuesets(
                    self.values.code.coding[i].code, get_url, "timing code"
                )
    ...
    def validate_fhir_sampleddata(self):
        if re.search(r"(\s\s)+", self.values.data):
            raise TypeError("data in sampleddata must not have more than 1 whitespace")
    ...
    def validate_fhir_address(self):
        ...
        if self.values.country:
            ...
            try:
                ...
                msg = (
                    "This country code is not recommended. "
    ...
    def validate_fhir_signature(self):
        ...
        if not (self.values.contentType in SIGNATURE_MIME_TYPES):
            raise ValueError(
                "The signature content type should be " f"one of {SIGNATURE_MIME_TYPES}"
            )
    ...
    def validate_fhir_identifier(self):
        if self.values.type and self.values.type.coding:
            ...
            for i, val in enumerate(self.values.type.coding):
                ...
                validate_valuesets(
                    self.values.type.coding[i].code, get_url, "identifier type"
                )
    ...
    def validate_fhir_organizationcontact(self):
        if self.values.purpose:
            ...
            for coding in codings:
                ...
                validate_valuesets(coding.code, url, "organization contact type")
